...
export function RouteMapTab2({ tripId }: RouteMapTab2Props) {
  const [map, setMap] = useState<google.maps.Map | null>(null);
  const [directionsRenderer, setDirectionsRenderer] = useState<google.maps.DirectionsRenderer | null>(null);
  const markersRef = useRef<google.maps.Marker[]>([]);

  // Route points from Liptovský Mikuláš to Košice
  const routePoints: RoutePoint[] = [
    // Start
    { name: "Liptovský Mikuláš", type: "hotel", lat: 49.0833, lng: 19.6167, description: "Starting point" },
    
    // Gas stations along the route
    { name: "Shell Station", type: "gas_station", lat: 49.0500, lng: 20.2800, address: "Near Poprad" },
    { name: "OMV Petrol Station", type: "gas_station", lat: 48.9500, lng: 20.8500, address: "Near Spišská Nová Ves" },
    { name: "Slovnaft", type: "gas_station", lat: 48.7500, lng: 21.1500, address: "Near Košice" },
    
    // Attractions
    { name: "Tatra National Park", type: "attraction", lat: 49.1667, lng: 20.0833, description: "Stunning mountain scenery and hiking trails" },
    { name: "Spiš Castle", type: "attraction", lat: 48.9989, lng: 20.7681, description: "One of Europe's largest castle complexes - UNESCO site" },
    { name: "Levoča Old Town", type: "attraction", lat: 49.0214, lng: 20.5878, description: "Medieval walled town - UNESCO heritage" },
    { name: "Slovak Paradise National Park", type: "attraction", lat: 48.9167, lng: 20.3833, description: "Gorges, waterfalls, and ladder trails" },
    
    // Restaurants in Košice
    { name: "Med Malina", type: "restaurant", lat: 48.7164, lng: 21.2611, description: "Modern Slovak cuisine - highly rated", address: "Hlavná 81, Košice" },
    { name: "Villa Regia", type: "restaurant", lat: 48.7208, lng: 21.2583, description: "Fine dining in historic setting", address: "Dominikánske námestie, Košice" },
    { name: "Republika Východu", type: "restaurant", lat: 48.7186, lng: 21.2594, description: "Trendy restaurant with local specialties", address: "Hlavná 68, Košice" },
    
    // Destination
    { name: "Košice", type: "hotel", lat: 48.7164, lng: 21.2611, description: "Destination - Hotel Bankov Košice" },
  ];
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle>מסלול הנסיעה: ליפטובסקי מיקולש → קושיצה</CardTitle>
          <CardDescription>
            מרחק: ~220 ק"מ | זמן נסיעה: ~3.5 שעות | תאריך: 5 בספטמבר 2026
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="h-[500px] rounded-lg overflow-hidden border">
            <MapView
              ...
              initialCenter={{ lat: 48.9, lng: 20.4 }}
              initialZoom={8}
            />
          </div>
