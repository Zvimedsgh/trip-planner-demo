...
export function RouteMapTab3({ tripId }: RouteMapTab3Props) {
  const [map, setMap] = useState<google.maps.Map | null>(null);
  const [directionsRenderer, setDirectionsRenderer] = useState<google.maps.DirectionsRenderer | null>(null);
  const markersRef = useRef<google.maps.Marker[]>([]);

  // Route points from Košice to Vienna
  const routePoints: RoutePoint[] = [
    // Start
    { name: "Košice", type: "hotel", lat: 48.7164, lng: 21.2611, description: "Starting point" },
    
    // Gas stations along the route
    { name: "OMV Petrol Station", type: "gas_station", lat: 48.5000, lng: 20.5000, address: "Near Rožňava" },
    { name: "Shell Station", type: "gas_station", lat: 48.1500, lng: 19.1500, address: "Near Hungarian border" },
    { name: "Slovnaft", type: "gas_station", lat: 48.0500, lng: 17.5000, address: "Near Bratislava" },
    
    // Attractions
    { name: "Aggtelek Caves", type: "attraction", lat: 48.4667, lng: 20.5000, description: "UNESCO karst cave system on Slovak-Hungarian border" },
    { name: "Eger Castle", type: "attraction", lat: 47.9026, lng: 20.3774, description: "Historic Hungarian fortress with stunning views" },
    { name: "Bratislava Old Town", type: "attraction", lat: 48.1486, lng: 17.1077, description: "Charming historic center with castle" },
    { name: "Schönbrunn Palace", type: "attraction", lat: 48.1847, lng: 16.3122, description: "Imperial summer residence - UNESCO site" },
    
    // Restaurants in Vienna
    { name: "Plachutta Wollzeile", type: "restaurant", lat: 48.2082, lng: 16.3738, description: "Famous for Tafelspitz (boiled beef)", address: "Wollzeile 38, Vienna" },
    { name: "Figlmüller", type: "restaurant", lat: 48.2108, lng: 16.3728, description: "Legendary Wiener Schnitzel", address: "Wollzeile 5, Vienna" },
    { name: "Café Central", type: "restaurant", lat: 48.2105, lng: 16.3649, description: "Historic Viennese coffee house", address: "Herrengasse 14, Vienna" },
    { name: "Naschmarkt", type: "restaurant", lat: 48.1986, lng: 16.3589, description: "Vibrant food market with diverse cuisines", address: "Naschmarkt, Vienna" },
    
    // Destination
    { name: "Vienna", type: "hotel", lat: 48.2082, lng: 16.3738, description: "Destination - DoubleTree By Hilton Vienna" },
  ];
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle>מסלול הנסיעה: קושיצה → וינה</CardTitle>
          <CardDescription>
            מרחק: ~440 ק"מ | זמן נסיעה: ~5.5 שעות | תאריך: 7 בספטמבר 2026
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="h-[500px] rounded-lg overflow-hidden border">
            <MapView
              ...
              initialCenter={{ lat: 48.45, lng: 18.8 }}
              initialZoom={7}
            />
          </div>
